...
class Controller {
  ...
  public function reads($views=[], $data = [])
  {
    $status = true;
    $open = false;
    $data['error-file'] = "";
    ...
    if (!$status) return file_get_contents($open);
    if (!$status) exit();

    $sc = "";
    foreach ($views as $view) {
      $open = 'app/views/'.$view.'.php';
      $sc .= file_get_contents($open);
    }
    return $sc;
  }
  ...
  public function array_sort($array, $find, $order=SORT_ASC)
  {
    ...
    if (count($array) > 0) {
      foreach ($array as $k => $v) {
        if (is_array($v)) foreach ($v as $k2 => $v2) if ($k2 == $find) $sortable_array[$k] = $v2;
        if (!is_array($v)) $sortable_array[$k] = $v;
      }
  ...
  public function array_group($array, $key1, $key2=false, $key3=false) {
    $return = [];
  ...
  public function non_ascii($string="") 
  {
    ...
    $string = preg_replace('/[\x00-\x1F\x80-\xFF]/', '', trim($string));
